var Game = {
    ...
    antCollisionGroup: null,
    spiderCollisionGroup: null,
    BeePaths: [],
    clickRate : 100,
    nextClick : 0,
    activeBeePath : null,
    ...
    ready: function() {
        ...
        // this.antCollisionGroup = game.physics.p2.createCollisionGroup();
        // this.spiderCollisionGroup = game.physics.p2.createCollisionGroup();
        ...
        // add Ants for debug
        var ant_count = 5; // map.properties['ants'];
        this.Ants.group = game.add.group(); // FIXME: should be unnecessary
        this.Ants.group.add(game.add.sprite(x, y));
        var ant_x = this.Flowers.list[0].x + 100;
        for(var i=0;i<ant_count;i++) {
            //ant_x = Math.floor(Math.random()*1000);
            var y = flowerSpawnY;
            this.addEntity(this.Ants, ant_x + i*100, y);
        }

        // add Spider and seeds for debug:
        this.Spiders.group = game.add.group();
        for(var i=0;i<10;i++) {
            this.addEntity(this.Spiders, i*100+100, 370);
        }
        ...
        //     this.Ants.list[id].body.setCollisionGroup(this.antCollisionGroup);
        // }
        // for(id in this.Spiders.list){
        //     this.Spiders.list[id].addCollisionGroup(this.spiderCollisionGroup, this.antCollisionGroup);
        // }
